...
This report is evidence-based and references the current codebase. Last updated after Phase 5 medium security hardening on 2026-08-10.
...
- Phase 4 broker ordering/missed-Redis-removal and Phase 5 WebSocket/Redis-outage abuse scenarios are deterministic component tests rather than live multi-worker/multi-backend outage-load runs; protected-download and socket work limits are per backend process and production proxy limits remain external.
...
| 5. Subscriber management interface/API | Complete | [`backend/app/api/routes/memberships.py`](backend/app/api/routes/memberships.py) `join_channel`, `leave_channel`, `list_members`, `list_pending_requests`, `create_invite`, `accept_invite`, `approve_member`, `add_member_direct`, `promote_member`, `demote_member`, `update_admin_permissions`, `remove_member`; Phase 5 covers targeted/generic invite races | Complex permission matrix; non-invite flows are not all concurrency-tested | Medium | Add integration tests for remaining join/approve/promote/demote/remove races only if requirements expand |
...
| 13. Testing | Mostly complete | P0 plus Phase 1–5 security suites cover auth/session, authorization, uploads/media, identifiers, broker desired state, bounded sync/download/socket work, invite races, and rate-limit failure behavior; [`scripts/verify_demo_flow.py`](scripts/verify_demo_flow.py) is a manual verifier | No frontend tests, no live broker/Redis outage integration tests, no load tests | High | Add at least one RabbitMQ/WebSocket integration test and one frontend smoke test; keep the demo verifier as a separate tool |
...
| Security | 75 | Session-bound auth, replay detection, absolute lifetime, one-time WebSocket tickets, distributed revocation, bounded established-socket work, atomic/fail-safe rate limiting, locked invites, encryption, and lifecycle-aware protected uploads exist; browser credential storage and production posture remain demo-grade |

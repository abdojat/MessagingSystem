...
- Phase 10 adds authenticated provider-independent mailbox verification with hash-only exact-email challenges and migration 0023. It also replaces per-socket online/offline toggles with atomic Redis connection leases, heartbeat refresh, and a bounded duplicate-safe reaper across backend instances.
...
| 5. Subscriber management interface/API | Complete | [`backend/app/api/routes/memberships.py`](backend/app/api/routes/memberships.py) covers the membership lifecycle; Phase 5 covers invite races; Phase 6 binds existing email targets to immutable accounts; Phase 10 supplies authenticated mailbox proof and proves denial-before/acceptance-after verification for unresolved targets | SMTP/provider availability is operational, and non-invite flows are not all concurrency-tested | Medium | Demonstrate either generic/user-ID invites or the configured capture/SMTP verification flow |

...
- Tamper-evident audit integrity through existing per-scope SHA-256 hash chains plus global bounded Merkle batches, compact inclusion proofs, Ed25519-signed checkpoint chaining, and optional external anchor export/verification.
...
- Migration `0024_phase11_merkle_audit` adds constrained batch and leaf-snapshot tables without rewriting existing audit rows or event hashes.
...
- Verified during Phase 11 Merkle audit integrity on 2026-08-11: the focused suite passed `40 passed`; Phase 8-10 passed `73 passed, 1 warning`; the complete backend suite passed `352 passed, 1 warning`; frontend typecheck/build and 845-key locale alignment passed; all three Compose renders passed and rendered production configuration placed the private signing seed only on `merkle-checkpoint`. Fresh `->0024` and representative `0023->0024` upgrades passed without changing existing event hashes or Phase 9/10 data. Live PostgreSQL checks proved a 16-event signed proof/tampered-copy failure, proof/anchor offline and database verification, a linked second checkpoint, concurrent non-overlap, payload/root corruption detection, and a 1,024-event/10-sibling performance path. The isolated hardened broker/WebSocket demo passed. At that historical checkpoint the canonical rebuild had not completed within the host's five-minute limit, so the demo used the prior dependency image with current source/migrations; the Phase 12 entry below records the canonical-build resolution.
- Verified during final Phase 12 stabilization on 2026-08-11: the canonical
  backend cold build completed in 252.11 seconds from the tracked Dockerfile,
  the warm source-only rebuild completed in 8.78 seconds with its dependency
  layer cached, and the raw image size fell from about 152.4 MB to 84.5 MB after
  removing the unused compiler and installing exact wheel-only locks. The full
  production Compose build and fresh empty-volume stack passed; Alembic reached
  the single `0024_phase11_merkle_audit` head and representative `0023 -> 0024`
  data remained unchanged. The requested focused matrix passed `213 passed` and
  the complete backend passed `352 passed, 2 warnings`; frontend
  typecheck/build and 845-key locale/placeholder parity passed. A disposable
  production E2E proved real captured SMTP verification/invite acceptance,
  private RBAC, RabbitMQ/worker/Redis/WebSocket delivery, encrypted message,
  outbox and attachment storage, offline sync isolation, two-backend aggregate
  presence, session replay/logout behavior, audit integrity, member-removal
  denial, signed Merkle checkpoints/proof/tampered-copy rejection, and offline
  plus database-backed external-anchor verification. This resolves the Phase 11
  canonical-build caveat.
...
- Phase 11 collects initialized event hashes into one bounded global Merkle-checkpoint sequence. Domain-separated binary SHA-256 hashing and unchanged odd-node promotion produce deterministic roots and explicit-side proofs.
- Each checkpoint signs canonical metadata with an isolated Ed25519 identity. The normal backend keeps only public verification keys; only the explicit one-shot maintenance profile receives the private seed, and the worker receives no Merkle keys.
- Superadmins can inspect status and verify an event proof in the bilingual admin console. The CLI also supports status, checkpoint, database verification, proof export/offline verification, and signed anchor export/verification.
...
- Signed Merkle checkpoints resist database-only rewriting without the private signing key, but they are not immutable storage. Rollback/deletion detection requires an exported latest anchor to be retained independently; this is an explicit operator action, not automatic notarization.
- A compromised backend can create audit events, signing-key compromise defeats future signatures, and no blockchain, distributed witness, external KMS/HSM, or multi-host checkpoint HA is implemented.

...
- Only explicit `dev`/`development`/`local`/`test` labels permit development secret behavior; every other environment label rejects missing, placeholder, weak, or invalid JWT/message-encryption secrets.

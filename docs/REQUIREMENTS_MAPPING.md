...
| Security: encryption, authentication, permissions | Complete | JWT auth, role/permission checks, Fernet encryption at rest, protected upload downloads, avatar upload access rules, safe avatar/routing identifiers, unauthorized security events | Login required routes, denied unauthorized actions, DB ciphertext query, avatar/upload regression tests |

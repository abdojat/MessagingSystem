...
| Security: encryption, authentication, permissions | Complete | Session-bound access JWTs; idle plus absolute session expiry; refresh-family replay detection; one-time Redis WebSocket tickets; distributed socket revocation; role/permission checks; Fernet encryption at rest; protected immutable streamed uploads; safe identifiers; migrations `0015_superadmin_controls` and `0017_auth_session_hardening`; focused tests in `backend/tests/security/test_phase1_hardening.py`, `backend/tests/security/test_phase2_auth_hardening.py`, and `backend/tests/test_superadmin.py` | Login required routes, revoke one session and show its old access token/socket rejected while another session remains active, then demonstrate denied unauthorized/pending actions and protected upload behavior |

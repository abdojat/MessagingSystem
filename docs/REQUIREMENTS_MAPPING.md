...
| Security: encryption, authentication, permissions | Complete | JWT auth, role/permission checks, Fernet encryption at rest, protected upload downloads, stored-upload checks before attachment publish, server-derived attachment metadata, SVG upload rejection, avatar upload access rules, safe avatar/routing identifiers, unauthorized/security/upload audit events | Login required routes, denied unauthorized actions, DB ciphertext query, avatar/upload/media regression tests |

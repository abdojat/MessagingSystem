...
| Security: encryption, authentication, permissions | Complete for university MVP; production-oriented single-host boundary validated | Session-bound access JWTs remain memory-only in the browser; refresh tokens use rotating `HttpOnly`, `Secure`, `SameSite` cookies with exact-Origin and double-submit CSRF validation. Phase 9 adds explicit-key-ID message v2 encryption, a bounded historical key ring, HKDF domain separation, chunked AES-256-GCM upload storage, fail-closed plaintext/integrity policy, migration `0022`, and bounded status/migration/rotation tooling. Backend authorization occurs before message/upload decryption; workers route encrypted payloads without data keys. Phase 1-9 regressions, schema upgrades, production configuration, and a fresh Phase 9 production-profile broker/WebSocket/storage-boundary verifier provide evidence. | Run `crypto_tool status`, demonstrate a v2 database envelope and encrypted upload storage marker absence, download as an authorized member, deny an outsider and encrypted Range request, then explain rotation/status. Publicly trusted certificates, external KMS/HSM, MFA, multi-host HA, and browser automation remain documented limitations. |

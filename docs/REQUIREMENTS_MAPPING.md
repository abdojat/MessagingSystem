...
| Security: encryption, authentication, permissions | Complete | JWT auth, role/permission checks, Fernet encryption at rest, protected and immutable streamed uploads, safe identifiers, production-like secret validation, `/sync` membership-event scoping, strict empty WebSocket subscription filtering, pending-member read denial, migration `0015_superadmin_controls`, guarded/non-cacheable `/v1/admin/*` console APIs, and focused tests in `backend/tests/security/test_phase1_hardening.py` plus `backend/tests/test_superadmin.py` | Login required routes, denied unauthorized/pending actions, upload a file then show second PUT rejected, DB ciphertext query, then show normal-user denial of the superadmin console |

...
| Security: encryption, authentication, permissions | Complete | Session-bound access JWTs; idle plus absolute session expiry; refresh-family replay detection; one-time Redis WebSocket tickets; distributed socket revocation; role/permission checks; Fernet encryption at rest; protected immutable streamed uploads plus streamed/concurrency-bounded downloads; indexed attachment authorization; safe identifiers; bounded `/sync`; versioned broker desired state; generation-aware pre-decryption WebSocket authorization; migrations `0015`, `0017`, `0018`, and `0019_phase4_p0_hardening`; focused Phase 1–4 security tests | Login required routes, revoke one session and show its old access token/socket rejected while another session remains active, then demonstrate denied unauthorized/pending actions, protected streaming upload behavior, and an oversized message rejection |

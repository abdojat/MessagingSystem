...
| RabbitMQ publish/subscribe architecture | Complete for defined MVP | Topic exchange/topology in backend and worker, durable bounded queues/bindings, retained legacy queue migration, versioned desired binding projection, confirms/acks | Delivery/reliability and routing regressions, retained-queue migration guards, live demo/release-candidate verifier | Explain DB/outbox -> RabbitMQ -> worker -> Redis path and optionally show management/logs. |

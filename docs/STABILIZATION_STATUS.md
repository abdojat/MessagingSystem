...
| WebSocket authentication | Fixed and verified | `POST /auth/ws-ticket`, cryptographically random 30-second default ticket, hashed Redis key, atomic `GETDEL`, access-JWT URL rejection, and socket authentication-expiry task | Redis is required to issue/consume tickets; browser access/refresh storage remains JavaScript-managed | Move browser auth to httpOnly cookies with CSRF design in a later phase |
...
| Security honesty | Checked | `docs/SECURITY.md` documents JS-managed browser credential storage as demo-grade and the Phase 2 one-time WebSocket-ticket/session-revocation design | Browser credential storage remains non-production despite hardened WebSocket transport | Do not claim production-grade browser session security |

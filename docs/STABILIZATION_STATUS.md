...
Security Hardening Phase 8 closes the browser-session and single-host production-boundary portions of AV-11. Browser refresh credentials now use rotating `HttpOnly`, `Secure`, `SameSite` cookies with exact-Origin and CSRF validation, while access tokens remain memory-only. A separate production Compose path adds TLS termination, security headers, internal-only services, non-root/read-only application containers, authenticated infrastructure, a separate least-privilege database role, and an explicit migration job. The complete backend suite, frontend checks, production image builds, live TLS/port/credential probes, and full RabbitMQ-to-WebSocket demo passed. Centralized secret management/KMS, publicly trusted certificate issuance, email delivery, cross-host HA/distributed limits, attachment encryption, and production load testing remain deferred.
...
| Area | Status | Evidence | Remaining Risk | Next Action |
| ---- | ------ | -------- | -------------- | ----------- |
| Email-targeted invite identity (P5V-01) | Fixed at the authorization/data boundary | Existing addresses resolve to authoritative `invited_user_id`; unresolved addresses require exact normalized email plus `email_verified_at`; `PATCH /me` clears proof on real address change; migration 0020 normalizes/backfills without verifying historical emails; reassignment and concurrent-claim regressions pass | No email delivery/token-completion provider is implemented; unresolved pre-registration acceptance is intentionally blocked until one exists | Add a real provider-backed verification completion flow only when deployment requirements demand it |

...
| ---- | ------ | -------- | -------------- | ----------- |
| Merkle algorithm and proofs | Implemented and verified | Binary domain separation (`0x00` leaf, `0x01` parent), unchanged odd-node promotion, explicit-side proof nodes, 64-sibling bound, malformed-hash rejection, and proof tests up to 1,025 leaves | Proof metadata is integrity evidence, not confidentiality; odd promotion means path length can vary | Use the deterministic demo to explain a 4-sibling proof for 16 leaves and 10 siblings for 1,024 leaves |
